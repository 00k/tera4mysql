...
static pthread_mutex_t tera_mutex;
static tera::Client* tera_client;
...
static int tera_init_func(void *p)
{
  DBUG_ENTER("tera_init_func");

  pthread_mutex_init(&tera_mutex, NULL);
...
Tera_share *ha_tera::get_share(const char* table_name)
{
  ...
  for (int i = 0; i < real_table_name.size(); i++)
  {
  ...
  {
    if (tera_client == NULL &&
        (tera_client = tera::Client::NewClient("/tmp/tera.flag", "../log/tera")) == NULL)
    {
      goto err;
    }
...
err:
  pthread_mutex_unlock(&tera_mutex);
...
*/
static st_handler_tablename ha_tera_system_tables[]= {
  {"meta_table", ""},
  {"stat_table", ""},
  {"user_table", ""},
  {(const char*)NULL, (const char*)NULL}
...
int ha_tera::write_row(uchar *buf)
{
  ...
  */
  ...
  tera::RowMutation* mu = share->table->NewRowMutation(key);
  mu->Put(value);
  share->table->ApplyMutation(mu);
...
int ha_tera::delete_row(const uchar *buf)
{
  ...
  tera::RowMutation* mu = share->table->NewRowMutation(last_key);
  mu->DeleteRow();
  share->table->ApplyMutation(mu);
  tera::ErrorCode ec = mu->GetError();
  delete mu;
...
int ha_tera::rnd_init(bool scan)
{
  ...
  std::cerr << "ha_tera::rnd_init" << std::endl;
  delete result_stream;

  tera::ErrorCode ec;
  tera::ScanDescriptor scan_desc("");
  result_stream = share->table->Scan(scan_desc, &ec);
...
int ha_tera::rnd_next(uchar *buf)
{
  ...
  std::cerr << "ha_tera::rnd_next" << std::endl;
  // MYSQL_READ_ROW_START(table_share->db.str, table_share->table_name.str, TRUE);

  std::string key, value;
  for (; !result_stream->Done(); result_stream->Next())
  {
    const std::string& key = result_stream->RowName();
    const std::string& value = result_stream->Value();
    if (key == last_key)
    {
      continue;
    }
    if (result_stream->Family() != "" || result_stream->Qualifier() != "")
    {
      continue;
    }
...
void ha_tera::position(const uchar *record)
{
  ...
  std::cerr << "ha_tera::position" << std::endl;
  memcpy(ref, last_key.c_str(), last_key.size());
  ref_length = last_key.size();
  DBUG_VOID_RETURN;
}
...
int ha_tera::rnd_pos(uchar *buf, uchar *pos)
{
  ...
  std::cerr << "ha_tera::rnd_pos" << std::endl;
  MYSQL_READ_ROW_START(table_share->db.str, table_share->table_name.str, TRUE);
  last_key.assign((char*)pos, ref_length);
                    ...
                    HA_CREATE_INFO *create_info)
{
  ...
  std::string real_table_name = name;
  for (int i = 0; i < real_table_name.size(); i++)
  {
    char c = real_table_name[i];
    if (c == '.' || c == '/')
  ...
  pthread_mutex_lock(&tera_mutex);
  tera::Client* tmp_client = get_tera_client();
  if (tmp_client == NULL)
  {
    pthread_mutex_unlock(&tera_mutex);
    DBUG_RETURN(-1);
  ...
  table_desc.SetRawKey(tera::kGeneralKv);
  table_desc.AddLocalityGroup("default");
  if (!tmp_client->CreateTable(table_desc, &ec))
  {
    pthread_mutex_unlock(&tera_mutex);
    DBUG_RETURN(-1);
  }
...
{
  {"var1", (char *)&tera_vars.var1, SHOW_LONG, SHOW_SCOPE_GLOBAL},
  {"var2", (char *)&tera_vars.var2, SHOW_DOUBLE, SHOW_SCOPE_GLOBAL},
  {0,0,SHOW_UNDEF, SHOW_SCOPE_UNDEF} // null terminator required
};

static struct st_mysql_show_var show_array_tera[]=
{
  {"array", (char *)show_status_tera, SHOW_ARRAY, SHOW_SCOPE_GLOBAL},
  {"var3", (char *)&tera_vars.var3, SHOW_CHAR, SHOW_SCOPE_GLOBAL},
  {"var4", (char *)&tera_vars.var4, SHOW_BOOL, SHOW_SCOPE_GLOBAL},
  {0,0,SHOW_UNDEF, SHOW_SCOPE_UNDEF}
};

static struct st_mysql_show_var func_status[]=
{
  {"tera_func_tera", (char *)show_func_tera, SHOW_FUNC, SHOW_SCOPE_GLOBAL},
  {"tera_status_var5", (char *)&tera_vars.var5, SHOW_BOOL, SHOW_SCOPE_GLOBAL},
  {"tera_status_var6", (char *)&tera_vars.var6, SHOW_LONG, SHOW_SCOPE_GLOBAL},
  {"tera_status",  (char *)show_array_tera, SHOW_ARRAY, SHOW_SCOPE_GLOBAL},
  {0,0,SHOW_UNDEF, SHOW_SCOPE_UNDEF}
};
